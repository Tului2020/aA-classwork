...
class Chirp < ApplicationRecord 
  ...
  Chirp.select(:id, :body, "count(*) as num_likes").joins(:likes).group(:id).as_json
  # Find chirps with at least 3 likes
  Chirp.select(:id, :body, "count(*) as num_likes").joins(:likes).group(:id).having("count(*) >= 3").as_json
