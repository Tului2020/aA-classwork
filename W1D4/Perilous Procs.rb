...
def filter_out(array, &proc)
    new_arr = []
    array.each { |el| new_arr << el if !proc.call(el)}
    new_arr
end
...
def filter_out!(array, &proc)
    array.map! { |el| el if !proc.call(el)}.delete(nil)
end
...
def multi_map(array, num=1, &proc)
    new_arr = array.dup
    num.times { new_arr.map! {|el| proc.call(el)} }
    new_arr
end
...
def proctition(arr, &prc)
    ret = [[], []]
    arr.each { |el| prc.call(el)? ret[0] << el : ret[1] << el}
    ret.flatten
end
...
def chain_map(num, proc)
    proc.inject(num) {|acc_proc, ele_proc| ele_proc.call(acc_proc) }
end
...
def proc_suffix(str, hsh)
    sentence = str.split(" ")
    hsh.each do |prc, val| 
        str.split(" ").each.with_index { |word, idx| sentence[idx] += val if prc.call(word) }
    end
    sentence.join(" ")
end
...
def proctition_platinum(arr, *procs)
    platnium_hash = Hash.new(0)
    used_indices = []
    procs.each do |prc|
        temp = []
        arr.each_with_index { |el, idx| temp << [el, idx] if prc.call(el) && !used_indices.include?(idx)}

        if temp.length > 0
            platnium_hash[platnium_hash.length + 1] = temp.transpose()[0]
            used_indices += temp.transpose()[1]
        else
            platnium_hash[platnium_hash.length + 1] = []
        end
    end
    platnium_hash
end
...
def procipher(str, hsh)
    sentence = str.split(" ")
    hsh.each do |prc, val| 
        str.split(" ").each.with_index { |word, idx| sentence[idx] = val.call(sentence[idx]) if prc.call(word) }
    end
    sentence.join(" ")
end
...
def picky_procipher(str, hsh)
    sentence = str.split(" ")
    used_indices = [] 
    hsh.each do |prc1, prc2| 
        temp = []
        str.split(" ").each.with_index { |word, idx| temp << [prc2.call(sentence[idx]), idx] if prc1.call(word) && !used_indices.include?(idx) }
        if temp.length > 0
            (0...temp.length).each { |i| sentence[temp[i][1]] = temp[i][0]} 
            used_indices += temp.transpose()[1]
        end
    end
    sentence.join(" ")
